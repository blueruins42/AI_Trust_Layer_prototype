...
- **数字字体统一（强约束）**：页面所有数字（指标卡 st.metric、图表数值、表格 #/响应时间、翻页页码、坐标轴、图例百分比）必须统一为单一字体；用户指定用黑体/无衬线（禁止 IBM Plex Mono 混用），用 `_NUM` 栈 `'Inter','PingFang SC','Heiti SC','Microsoft YaHei',sans-serif` 并通过全局 CSS 强制 st.metric 数值