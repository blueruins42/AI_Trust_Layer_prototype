# PRD Step 2: User Personas & Requirements Analysis (Revised v2)
...
> *(Same as v1, not repeated here. Three roles: Li Ming — primary user; Wang Fang — secondary user; Engineer Zhang — non-target user)*
...
|--------------|--------------------------------------|
| A toy with looks only | An industrial-grade product that accounts for the business loop, data iteration, and engineering feasibility |
| A one-way presentation layer | A Human-AI Co-evolution System |
...
The v1 user journey only showed the perfect scenario where the AI gives "high confidence." But the core of HCAI is solving **"uncertainty."** If the confidence of the data retrieved by the AI is extremely low (say only 30%), how should the system present it to Li Ming?
...
{
  "answer": {
    "text": "本项目需配置 ZDJ-200 型转辙机 12 台，每台含安装支架及配套线缆。",
  ...
  "sources": [
    {
      "document_name": "XX项目技术规格书 v3.2",
      "page_number": 15,
      "match_score": 0.92,
      "excerpt": "本项目信号系统采用 ZDJ-200 型电动转辙机..."
    },
    {
      "document_name": "XX项目信号系统设计图",
      "page_number": 8,
      "match_score": 0.78,
      "excerpt": "转辙机布置图：共12处道岔..."
    }
  ],
  "jargon_glossary": [
    {
      "term": "转辙机",
      "definition": "控制铁路道岔转换的机械设备",
      "plain_language": "让火车换轨道的开关"
    },
    {
      "term": "弱电集成",
      "definition": "低压电气系统的综合集成工程",
      "plain_language": "把通信、信号、监控等低压系统打包在一起做"
    }
  ],
  "verification_advice": {
    "needs_verification": true,
    "fields_to_check": ["设备数量需与图纸交叉验证"],
    "action_link": {
      "text": "查看图纸第8页",
      "document": "XX项目信号系统设计图",
      "page": 8
    }
  },
  "metadata": {
...
4. **Academically citable**: The MSc thesis can present this data contract as a "design contribution"
...
| **Engineer Zhang's cry** | "You want to both generate the answer, compute confidence, AND extract jargon! If you use too complex a model, the API response takes 15 seconds and the server will explode!" |
...
| **Engineer Zhang's cry** | "Don't talk to me about feelings! Tell me exactly what JSON format the backend should pass!" |
...
| **Engineer Zhang's cry** | "If every query calls GPT-4 three times for prompt validation, the company's API bill will crash!" |
...
## 2.8 Next Step Preview

Proceed to **Step 3: Product Vision & PRD Core Framework**

- Product vision statement
- Product positioning
- Success metrics
- MVP scope definition (based on v2 priority matrix)
